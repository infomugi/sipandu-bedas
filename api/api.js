...
app.get('/api/laporan/gizi', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(
            `-- ⚡ Bolt Optimization: Compute expensive AGE() function exactly once using a CTE
             WITH raw_data AS (
                 SELECT a.nama_lengkap,
                 AGE(sk.tgl_pelayanan, a.tanggal_lahir) AS interval_umur,
             ...
             SELECT nama_lengkap,
             (EXTRACT(YEAR FROM interval_umur)*12 + EXTRACT(MONTH FROM interval_umur)) AS umur_bulan,
