...
app.get('/api/laporan/dashboard', async (req, res) => {
    try {
        ...
        const [distribusi, ringkasanRt, keluargaAbsen] = await Promise.all([
            ...
            pool.query(
                // ⚡ Bolt Optimization: Using LEFT JOIN LATERAL to calculate MAX(tgl_kunjungan)
                // exactly once per family. This eliminates 3 redundant correlated subqueries
                // in SELECT and HAVING, and removes the need for an expensive GROUP BY.
                `SELECT k.no_kk, a.nama_lengkap AS kepala_keluarga, k.rt, k.rw,
                 ...
                 WHERE k.is_aktif=1 ${kader_id ? 'AND k.kader_id=$1' : ''}
                 AND (kj.kunjungan_terakhir IS NULL OR kj.kunjungan_terakhir < CURRENT_DATE - INTERVAL '3 months')
                 ORDER BY kj.kunjungan_terakhir`, kader_id ? [kader_id] : []
            )
        ]);
