...
app.get('/api/keluarga/:keluarga_id/anggota', async (req, res) => {
    try {
        const { rows } = await pool.query(
            `-- ⚡ Bolt Optimization: Extracted expensive AGE() and EXTRACT() functions into a CTE.
             -- This prevents redundant calculations of umur_tahun and umur_bulan_mod across multiple SELECT and CASE clauses, reducing CPU overhead.
             WITH CalculatedAge AS (
                 SELECT *,
