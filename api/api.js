...
// 🛡️ Sentinel: Secure password hashing functions
...
const comparePassword = (password, hashed) => {
    return new Promise((resolve, reject) => {
        if (!hashed || !hashed.startsWith('$pbkdf2$')) {
            // Support secure migration of legacy plaintext passwords
            resolve({ isValid: String(password) === String(hashed), needsUpgrade: true });
            return;
        }
        ...
        crypto.pbkdf2(password, salt, 100000, 64, 'sha512', (err, derivedKey) => {
            ...
            resolve({ isValid: crypto.timingSafeEqual(derivedKey, keyBuffer), needsUpgrade: false });
        });
    });
};
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (!isValid) return err(res, 'Password salah', 401); // 🛡️ Sentinel: Fix authentication bypass

        // If the password was valid but needs an upgrade (from plaintext to hashed), update it silently
        if (needsUpgrade) {
            const newHash = await hashPassword(password);
            await pool.query('UPDATE users SET password = $1 WHERE id = $2', [newHash, rows[0].id]);
        }
...
app.put('/api/profil/:id/password', async (req, res) => {
    try {
        ...
        await pool.query('UPDATE users SET password=$1, updated_by=$2 WHERE id=$3', [newHashedPassword, updated_by, req.params.id]);
