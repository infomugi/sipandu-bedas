...
app.get('/api/keluarga/:keluarga_id/anggota', async (req, res) => {
    try {
        // ⚡ Bolt Optimization: Calculated expensive AGE() and EXTRACT() functions once in a CTE.
        // This avoids repeating these calculations multiple times in the SELECT and CASE clauses, improving database performance.
        const { rows } = await pool.query(
            `WITH base_anggota AS (
                 SELECT *,
