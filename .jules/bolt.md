...
## 2024-05-18 - Caching Database-Backed Middleware
**Learning:** Authorization middlewares that execute database queries (like checking a user's role) on every request can become significant performance bottlenecks, especially for high-traffic or frequent polling endpoints.
**Action:** Implement an LRU-like in-memory Map with a TTL inside the middleware to cache roles, dramatically reducing DB hits while maintaining security. Always remember to invalidate this cache in endpoints that update the relevant user state (like profiles or active status).
